...
export default function ProductCard({ product } : ProductCardProps) {
   ...
   return (
      ...
      />
      <div className="p-3">
         <h4 className="text-lg font-semibold uppercase text-gray-800">{title}</h4>
         <p>{description}</p>
      </div>
      <a href={`/product/${id}`} className="mt-auto">
         <button
            className="bg-green-900 p-3 w-full rounded mt-2 hover:bg-green-800 duration-200 text-green-200 font-bold text-lg"
         >
            Ver más
         </button>
      </a>
